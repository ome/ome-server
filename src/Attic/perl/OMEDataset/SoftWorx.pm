...
# Also note that some data is read from the file using a closed-source, architechture-dependent library,
# which may not be available on all systems.
# Specifically, this includes reading the time information from API's "Extended Header" - a piece of info
# common to all 5-D data.
...
# Conveniently, the attribute names match the Object's field names so me don't need a map.
	my $command = "$DumpSoftWorxHeader $datasetPath 2> $tempFileNameErr |";
	open (STDOUT_PIPE,$command);
	while (<STDOUT_PIPE>) {
		chomp;
		($attribute,$value) = split (/\t/,$_);
	# trim leading and trailing whitespace on $attribute and $value
...
# THIS IS A CLOSED-SOURCE PROPRIETARY PROGRAM NOT NECESSARILY AVAILABLE ON ALL SYSTEMS!!!!!
# The output is one line per XY plane.
	if (-x $DumpSoftWorxExtHeader) {
		my $command = "$DumpSoftWorxExtHeader $datasetPath 2>> $tempFileNameErr |";
		open (STDOUT_PIPE,$command);
		@columns = split (/\t/, <STDOUT_PIPE>);
		while (<STDOUT_PIPE>) {
			chomp;
			@columns = split (/\t/, $_);
			foreach (@columns) {$_ =~ s/^\s+//;$_ =~ s/\s+$//;} # Trim leading and trailing whitespace
			($waveNum,$timePoint,$zSection) = ($columns[1],$columns[2],$columns[0]);
			...
			$XYinfo->[$waveNum][$timePoint][$zSection]->{Min}          = $columns[8];
			$XYinfo->[$waveNum][$timePoint][$zSection]->{Max}          = $columns[9];
			...
			$Wavelengths->[$waveNum]->{ExWavelength}                   = $columns[13];
			$Wavelengths->[$waveNum]->{EmWavelength}                   = $columns[14];
...
# The file will actually be read at this point, and an error reported if its corrupt.
	my $command = "$DumpSoftWorxStats $datasetPath 2>> $tempFileNameErr |";
	open (STDOUT_PIPE,$command);
	@columns = split (/\t/, <STDOUT_PIPE>);
	while (<STDOUT_PIPE>) {
		chomp;
		...
		$XYZinfo->[$waveNum][$timePoint]->{Min}       = $columns[ 3];
		$XYZinfo->[$waveNum][$timePoint]->{Max}       = $columns[ 4];
		...
		for ($time=0;$time<$numTimes;$time++) {
			for ($zSection=0;$zSection<$numZ;$zSection++) {
				$sth->execute($ID,$wave,$time,$zSection,
					$XYinfo->[$wave][$time]->{DeltaTime},
					$XYinfo->[$wave][$time]->{ExposureTime},
					$XYinfo->[$wave][$time]->{StageX},
					$XYinfo->[$wave][$time]->{StageY},
					$XYinfo->[$wave][$time]->{StageZ},
					$XYinfo->[$wave][$time]->{Min},
					$XYinfo->[$wave][$time]->{Max},
					$XYinfo->[$wave][$time]->{Mean},
					$XYinfo->[$wave][$time]->{GeoMean},
					$XYinfo->[$wave][$time]->{Sigma}
				);
			}
		}
	}
...
# Insert the SoftWorx-specific stuff.
	...
	for ($wave=0;$wave<$numWaves;$wave++) {
		for ($time=0;$time<$numTimes;$time++) {
			for ($zSection=0;$zSection<$numZ;$zSection++) {
				$sth->execute($ID,$wave,$time,$zSection,
					$XYinfo->[$wave][$time]->{Photosensor},
					$XYinfo->[$wave][$time]->{IntenScaling}
				);
			}
		}
	}

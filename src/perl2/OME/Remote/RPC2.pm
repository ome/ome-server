...
sub _item {
    ...
    } elsif ($ref eq 'OME::Remote::RPC2::Boolean') {
	push @text, "<value><boolean>", $item->repr, "</boolean></value>\n";
    } elsif ($ref eq 'OME::Remote::RPC2::String') {
      push @text, "<value><string>", $item->repr, "</string></value>\n";
    } elsif ($ref eq 'OME::Remote::RPC2::Integer') {
      push @text, "<value><int>", $item->repr, "</int></value>\n";
    } elsif ($ref eq 'OME::Remote::RPC2::Double') {
      push @text, "<value><double>", $item->repr, "</double></value>\n";
    } elsif ($ref eq 'OME::Remote::RPC2::DateTime::ISO8601') {
...
sub base64 {
    ...
    return OME::Remote::RPC2::Base64->new(@_);
}
...
sub boolean {
    ...
    my $elem = shift;
    if($elem == 0 or $elem == 1) {
        return OME::Remote::RPC2::Boolean->new($elem);
    } else {
...
sub double {
    ...
    if($elem =~ /^([+-]?)(?=\d|\.\d)\d*(\.\d*)?([Ee]([+-]?\d+))?$/) {
        return OME::Remote::RPC2::Double->new($elem);
    } else {
...
sub int {
    ...
    # this is from `perldata(1)'
    if($elem =~ /^[+-]?\d+$/) {
        return OME::Remote::RPC2::Integer->new($elem);
    } else {
...
sub string {
    ...
    return OME::Remote::RPC2::String->new(@_);
}

sub date_time {
    my $self = shift;

    return OME::Remote::RPC2::DateTime::ISO8601->new(@_);
}
...
sub start {
	...
	if ($tag eq 'methodCall') {
	    ...
	    OME::Remote::RPC2::die($expat, "unknown RPC type \`$tag'\n");
	}
    } elsif ($state eq 'want_method_name') {
	OME::Remote::RPC2::die($expat, "wanted \`methodName' tag, got \`$tag'\n")
	...
	OME::Remote::RPC2::die($expat, "wanted \`params' tag, got \`$tag'\n")
	    if ($tag ne 'params');
	push @{ $expat->{'rpc_state'} }, 'params';
    } elsif ($state eq 'params') {
	OME::Remote::RPC2::die($expat, "wanted \`param' tag, got \`$tag'\n")
	    if ($tag ne 'param');
	...
	if ($tag eq 'value') {
	    ...
	    OME::Remote::RPC2::die($expat, "wanted \`value' or \`name' tag, got \`$tag'\n");
	}
    } elsif ($state eq 'param_name') {
	OME::Remote::RPC2::die($expat, "wanted parameter name data, got tag \`$tag'\n");
    } elsif ($state eq 'want_value') {
	OME::Remote::RPC2::die($expat, "wanted \`value' tag, got \`$tag'\n")
	    if ($tag ne 'value');
	$expat->{'rpc_text'} = "";
	$expat->{'may_get_cdata'} = 1;
	push @{ $expat->{'rpc_state'} }, 'value';
	...
	if ($tag eq 'array') {
	    ...
	    OME::Remote::RPC2::die($expat, "wanted a data type, got \`$tag'\n");
	}
    } elsif ($state eq 'want_data') {
	OME::Remote::RPC2::die($expat, "wanted \`data', got \`$tag'\n")
	    if ($tag ne 'data');
	...
	OME::Remote::RPC2::die($expat, "wanted \`member' tag, got \`$tag'\n")
	    if ($tag ne 'member');
	push @{ $expat->{'rpc_state'} }, 'want_member_name';
    } elsif ($state eq 'want_member_name') {
	OME::Remote::RPC2::die($expat, "wanted \`name' tag, got \`$tag'\n")
	    if ($tag ne 'name');
	push @{ $expat->{'rpc_state'} }, 'member_name';
	$expat->{'rpc_text'} = "";
    } elsif ($state eq 'member_name') {
	OME::Remote::RPC2::die($expat, "wanted data, got tag \`$tag'\n");
    } elsif ($state eq 'cdata') {
	OME::Remote::RPC2::die($expat, "wanted data, got tag \`$tag'\n");
    } else {
	OME::Remote::RPC2::die($expat, "internal error, unknown state \`$state'\n");
    }
}
...
sub end {
	...
	if ($tag eq 'base64') {
	    $value = OME::Remote::RPC2::Base64->new($value);
	} #elsif ($tag eq 'boolean') {
		    #$value = OME::Remote::RPC2::Boolean->new($value);
	#} 
	elsif ($tag eq 'dateTime.iso8601') {
	    $value = OME::Remote::RPC2::DateTime::ISO8601->new($value);
	} elsif ($expat->{'use_objects'}) {
	    if ($tag eq 'i4' or $tag eq 'int') {
		$value = OME::Remote::RPC2::Integer->new($value);
	    } elsif ($tag eq 'float') {
		$value = OME::Remote::RPC2::Float->new($value);
	    } elsif ($tag eq 'string') {
		$value = OME::Remote::RPC2::String->new($value);
	    }
	}
...
package OME::Remote::RPC2::DataType;
...
package OME::Remote::RPC2::Base64;

use vars qw{@ISA};
@ISA = qw{OME::Remote::RPC2::DataType};

package OME::Remote::RPC2::Boolean;

use vars qw{@ISA};
@ISA = qw{OME::Remote::RPC2::DataType};

package OME::Remote::RPC2::Integer;

use vars qw{@ISA};
@ISA = qw{OME::Remote::RPC2::DataType};

package OME::Remote::RPC2::String;
...
sub repr {
    my $self = shift;
    my $value = $$self;
    $value =~ s/([&<>\"])/$OME::Remote::RPC2::char_entities{$1}/ge;
    $value;
}

package OME::Remote::RPC2::Double;

use vars qw{@ISA};
@ISA = qw{OME::Remote::RPC2::DataType};

package OME::Remote::RPC2::DateTime::ISO8601;
...
OME::Remote::RPC2 - encode/decode RPC2 format XML

=head1 SYNOPSIS

 use OME::Remote::RPC2;

 $coder = OME::Remote::RPC2->new;
...
I<OME::Remote::RPC2> encodes and decodes XML RPC calls.

=over 4

=item $coder = OME::Remote::RPC2->new( I<OPTIONS> )
...
example:
 ...
 $server = OME::Remote::RPC2->new( 'encoding' => 'ISO-8859-1' );
...
are `C<OME::Remote::RPC2::Boolean>',
`C<OME::Remote::RPC2::DateTime::ISO8601>', and
`C<OME::Remote::RPC2::Base64>'.

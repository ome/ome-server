...
sub serve {
    ...
    if (!OME::Remote::Facade->hasMethod($method)) {
        return $self->encode_fault(3, "no such method \`$method'\n");
    }

    my $result;
    my $eval = eval { 
    	$result = OME::Remote::Facade->$method(@{ $call->{'value'} }) };
    if ($@) {
	return $self->encode_fault(4, "error executing RPC \`$method'.\n" . $@);
    }
    
    my $response_xml;
    if (ref($result) eq 'OME::Remote::Response::XMLRPC') {
	$response_xml = $$result;	
    } else {
	$response_xml = $self->encode_response($result);
    }
    return $response_xml;
}
...
sub base64 {
    ...
    return FrontierTest::RPC2::Base64->new(@_);
}
...
sub boolean {
    ...
    my $elem = shift;
    if($elem == 0 or $elem == 1) {
        return FrontierTest::RPC2::Boolean->new($elem);
    } else {
...
sub double {
    ...
    if($elem =~ /^([+-]?)(?=\d|\.\d)\d*(\.\d*)?([Ee]([+-]?\d+))?$/) {
        return FrontierTest::RPC2::Double->new($elem);
    } else {
...
sub int {
    ...
    # this is from `perldata(1)'
    if($elem =~ /^[+-]?\d+$/) {
        return FrontierTest::RPC2::Integer->new($elem);
    } else {
...
sub string {
    ...
    return FrontierTest::RPC2::String->new(@_);
}

sub date_time {
    my $self = shift;

    return FrontierTest::RPC2::DateTime::ISO8601->new(@_);
}
...
sub start {
	...
	if ($tag eq 'methodCall') {
	    ...
	    FrontierTest::RPC2::die($expat, "unknown RPC type \`$tag'\n");
	}
    } elsif ($state eq 'want_method_name') {
	FrontierTest::RPC2::die($expat, "wanted \`methodName' tag, got \`$tag'\n")
	...
	FrontierTest::RPC2::die($expat, "wanted \`params' tag, got \`$tag'\n")
	    if ($tag ne 'params');
	push @{ $expat->{'rpc_state'} }, 'params';
    } elsif ($state eq 'params') {
	FrontierTest::RPC2::die($expat, "wanted \`param' tag, got \`$tag'\n")
	    if ($tag ne 'param');
	...
	if ($tag eq 'value') {
	    ...
	    FrontierTest::RPC2::die($expat, "wanted \`value' or \`name' tag, got \`$tag'\n");
	}
    } elsif ($state eq 'param_name') {
	FrontierTest::RPC2::die($expat, "wanted parameter name data, got tag \`$tag'\n");
    } elsif ($state eq 'want_value') {
	FrontierTest::RPC2::die($expat, "wanted \`value' tag, got \`$tag'\n")
	    if ($tag ne 'value');
	$expat->{'rpc_text'} = "";
	$expat->{'may_get_cdata'} = 1;
	push @{ $expat->{'rpc_state'} }, 'value';
	...
	if ($tag eq 'array') {
	    ...
	    FrontierTest::RPC2::die($expat, "wanted a data type, got \`$tag'\n");
	}
    } elsif ($state eq 'want_data') {
	FrontierTest::RPC2::die($expat, "wanted \`data', got \`$tag'\n")
	    if ($tag ne 'data');
	...
	FrontierTest::RPC2::die($expat, "wanted \`member' tag, got \`$tag'\n")
	    if ($tag ne 'member');
	push @{ $expat->{'rpc_state'} }, 'want_member_name';
    } elsif ($state eq 'want_member_name') {
	FrontierTest::RPC2::die($expat, "wanted \`name' tag, got \`$tag'\n")
	    if ($tag ne 'name');
	push @{ $expat->{'rpc_state'} }, 'member_name';
	$expat->{'rpc_text'} = "";
    } elsif ($state eq 'member_name') {
	FrontierTest::RPC2::die($expat, "wanted data, got tag \`$tag'\n");
    } elsif ($state eq 'cdata') {
	FrontierTest::RPC2::die($expat, "wanted data, got tag \`$tag'\n");
    } else {
	FrontierTest::RPC2::die($expat, "internal error, unknown state \`$state'\n");
    }
}
...
sub end {
	...
	if ($tag eq 'base64') {
	    $value = FrontierTest::RPC2::Base64->new($value);
	} elsif ($tag eq 'boolean') {
	    $value = FrontierTest::RPC2::Boolean->new($value);
	} elsif ($tag eq 'dateTime.iso8601') {
	    $value = FrontierTest::RPC2::DateTime::ISO8601->new($value);
	} elsif ($expat->{'use_objects'}) {
	    if ($tag eq 'i4' or $tag eq 'int') {
		$value = FrontierTest::RPC2::Integer->new($value);
	    } elsif ($tag eq 'float') {
		$value = FrontierTest::RPC2::Float->new($value);
	    } elsif ($tag eq 'string') {
		$value = FrontierTest::RPC2::String->new($value);
	    }
	}
	...
	if ($expat->{'may_get_cdata'}) {
		...
		$expat->{'rpc_value'}
		= FrontierTest::RPC2::String->new($expat->{'rpc_text'});
	    } else {
...
package FrontierTest::RPC2::DataType;
...
package FrontierTest::RPC2::Base64;

use vars qw{@ISA};
@ISA = qw{FrontierTest::RPC2::DataType};

package FrontierTest::RPC2::Boolean;

use vars qw{@ISA};
@ISA = qw{FrontierTest::RPC2::DataType};

package FrontierTest::RPC2::Integer;

use vars qw{@ISA};
@ISA = qw{FrontierTest::RPC2::DataType};

package FrontierTest::RPC2::String;
...
sub repr {
    my $self = shift;
    my $value = $$self;
    $value =~ s/([&<>\"])/$FrontierTest::RPC2::char_entities{$1}/ge;
    $value;
}

package FrontierTest::RPC2::Double;

use vars qw{@ISA};
@ISA = qw{FrontierTest::RPC2::DataType};

package FrontierTest::RPC2::DateTime::ISO8601;
...
FrontierTest::RPC2 - encode/decode RPC2 format XML

=head1 SYNOPSIS

 use FrontierTest::RPC2;

 $coder = FrontierTest::RPC2->new;
...
I<FrontierTest::RPC2> encodes and decodes XML RPC calls.

=over 4

=item $coder = FrontierTest::RPC2->new( I<OPTIONS> )
...
example:
 ...
 $server = FrontierTest::RPC2->new( 'encoding' => 'ISO-8859-1' );
...
are `C<FrontierTest::RPC2::Boolean>',
`C<FrontierTest::RPC2::DateTime::ISO8601>', and
`C<FrontierTest::RPC2::Base64>'.
...
perl(1), FrontierTest::Daemon(3), FrontierTest::Client(3)

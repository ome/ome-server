...
sub createDatabase {
	...
	if (not defined $dbh->do(qq{CREATE DATABASE "$dbName"})) {
...
# $sth->execute($reloid)
use constant FIND_TABLE_BY_NUMBER_SQL => <<SQL;
	SELECT lower(relname)
	FROM pg_class
	WHERE oid = ?
SQL
# add
...
# returns an array with relation OID and relation kind
# my ($reloid,$relkind) = __findRelation ($dbh,'my_table_name');
# my ($reloid,$relkind) = __findRelation ($dbh,'my_sequence_name');
sub __findRelation {
	my ($self,$dbh,$rel) = @_;
	return $dbh->selectrow_array(FIND_RELATION_SQL,{},$rel);
}


# returns 1/undef if tables exists/doesn't.
sub tableExists {
	my ($self,$dbh,$rel) = @_;
	my ($reloid,$relkind) = $self->__findRelation ($dbh,$rel);
	return 1 if $reloid and $relkind = 'r';
	return undef;
}
...
# {$column} = [$foreign_table_name,$foreign_table_column_name]
# dies if table doesn't exist.

use constant FIND_TABLE_FOREIGN_KEYS_SQL => <<SQL;
	SELECT tgnargs AS number_args,
		   tgargs  AS args
	  FROM pg_trigger
...
sub __findForeignKeys {
	...
	# Get the table OID
	my ($reloid,$relkind) = $self->__findRelation ($dbh,$table);
	die "PostgresDelegate->__findForeignKeys():  Table not found" unless $reloid;
	
	my $find_column_by_number = $dbh->prepare_cached(FIND_COLUMN_BY_NUMBER_SQL);
	my $find_table_by_number  = $dbh->prepare_cached(FIND_TABLE_BY_NUMBER_SQL);

	my $sth_Foreign_Keys      = $dbh->prepare(FIND_TABLE_FOREIGN_KEYS_SQL);
	...
	while ( my $forcols = $sth_Foreign_Keys->fetchrow_hashref ) {
		...
		( undef, undef, $ftable, undef, @columns ) =
			split( /\000/, $args );

		# Account for old versions which don't handle NULL
		# but instead return a string of the escape sequence
		( undef, undef, $ftable, undef, @columns ) = split( /\\000/, $args )
			unless defined $ftable;
		...
		# If the number of columns > 1, then this isn't
		# the kind of FK we're looking for.
		my ($key,$fkey) = ($keylist[0],$fkeylist[0]);
		$table_Fkeys->{$fkey} = [ $ftable, $key ] if scalar @keylist == 1;
...
sub dropColumn {
	...
	# Get the table oid		
	my ($reloid)  = $self->__findRelation($dbh,$table);
...
sub notNULL () {
	my ($self,$dbh,$table,$column) = @_;
	die "notNULL: Wrong number of parameters"
		unless $dbh and $table and $column;
	$table = lc ($table);
	$column = lc ($column);
	...
	# Postgres >= 7.3 implements adding not NULL in SQL
	if ($db_version ge '7.3') {
		$dbh->do("ALTER TABLE $table ALTER COLUMN $column SET NOT NULL") or die $dbh->errstr();
	# In Postgres < 7.3 we have to do stuff manually.
	} else {
		$dbh->do(<<SQL)  or die $dbh->errstr();
			UPDATE pg_attribute SET attnotnull = TRUE WHERE attname = '$column'
			AND attrelid = ( SELECT oid FROM pg_class WHERE relname = '$table' )
SQL
	}
}
...
sub __createSequence {
	...
	#print "\n$sequence\n------------------\n";
	my ($reloid,$relkind)  = $self->__findRelation($dbh,$sequence);
...
sub __createIndex {
	...
	if (not ($ome_idx or $sys_idx)) {
		...
		until ($index_good) {
			...
			my ($reloid,$relkind)  = $self->__findRelation($dbh,$index_name);
...
sub addForeignKeyConstraints {
	...
	die "addForeignKeyConstraints: Wrong number of parameters"
		unless defined $dbh && defined $class;

	my $class_name;
	if (ref ($class)) {
		$class_name = ref ($class);
		die "addForeignKeyConstraints: Malformed class name $class"
			unless $class_name =~ /^\w+(\:\:\w+)*$/;
	} else {
		$class_name = $class;
		die "addForeignKeyConstraints: Malformed class name $class"
	...
	die "addForeignKeyConstraints: $class is not a subclass of OME::DBObject"
		unless UNIVERSAL::isa($class,"OME::DBObject");
	...
	while (my ($alias,$colSpec) = each %$columns) {
		...
		if ($foreign_class_name) {
			...
			die "PostgresDelegate->addForeignKeyConstraints(): Class $foreign_class_name ".
				"referenced from $class has no primary key table!" unless $ftable;

			# Get the foreign key definitions for this table (unless we already did it)
			$fKeys->{$table} = $self->__findForeignKeys ($dbh,$table)
				unless exists $fKeys->{$table};

			# Check if a FK constraint exists in the table's FK defs.
			if ( (not exists $fKeys->{$table}->{$column}) or
				($fKeys->{$table}->{$column}->[0] ne $ftable) or
				($fKeys->{$table}->{$column}->[1] ne $fcolumn) ) {
					my ($className,$fClassName) = ($class->getFormalName(),$foreign_class->getFormalName());
					$dbh->do (<<SQL);
						ALTER TABLE $table ADD CONSTRAINT "$className.$alias->$fClassName"
							FOREIGN KEY ($column) REFERENCES $ftable ($fcolumn)
			...
			if (defined $relkind) {
				...
				if (defined $primaries->{$table}) {
					...
					if (scalar(@pk_nums) == 1) {
						...
						$self->notNULL($dbh, $table, $dbo_primary_column);

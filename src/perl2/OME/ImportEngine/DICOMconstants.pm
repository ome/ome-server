...
# From http://dicomperl.sourceforge.net/doc.html
#
# Modified by Tom Macura (tmacura@nih.gov) for inclusion into OME.
# condensed Andrew's DICOM_fields.pm, DICOM_VRfields.pm and DICOM_private.pm into one.

package OME::ImportEngine::DICOMconstants;
...
our @dicom_private = (<<END_DICOM_PRIVATE =~ m/^\s*(.+)/gm);
# Example format:
# 0000   0000   UL   1      GroupLength
END_DICOM_PRIVATE

# Definitions of fields of DICOM headers.
our @dicom_fields = (<<END_DICOM_FIELDS =~ m/^\s*(.+)/gm);
0000   0000   UL   1      GroupLength

...
#print STDERR ref($self)."->createOMEPage()\nURL='".$cgi->url(-query=>1)."\n";
	...
	if( !(defined $ImageID || defined $DatasetID) ) {
		return ('ERROR', "OME::Web::GetGraphics needs ImageID or DatasetID as url parameters. Neither was supplied.");
	}
	if( defined $ImageID ) {
		my $image = $self->Factory()->loadObject("OME::Image",$ImageID);
		return ('ERROR', "Image (ID=$ImageID) not found in Database. This message generated by OME::Web::GetGraphics->createOMEpage()." )
			unless defined $image;
	}
	if( defined $DatasetID ) {
		my $dataset = $self->Factory()->loadObject("OME::Dataset",$DatasetID);
		return ('ERROR', "Dataset (ID=$DatasetID) not found in Database. This message generated by OME::Web::GetGraphics->createOMEpage()." )
...
sub DrawDatasetControl {
	...
	my $cgi   = $self->CGI();
	my $DatasetID = $cgi->url_param('DatasetID') || die "\nDataset id not supplied to GetGraphics.pm ";
	my $Dataset = $self->Factory()->loadObject("OME::Dataset",$DatasetID) || die "\nInvalide dataset id provided to GetGraphics.pm\n";

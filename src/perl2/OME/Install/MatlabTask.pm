...
sub execute {
	...
	if ($MATLAB->{INSTALL}) {
		...
		# write out ome-Matlab.h
		print "  \\_ Writing ome-Matlab.h";
		
		my $ome_matlab_h = <<'OME_MATLAB_H_START';
/*
	Auto Generated by the OME Installer. The contents of this header file
...
OME_MATLAB_H_START
		...
		if ($MATLAB_INFO{"VERS"} =~ /7\.[0,1,2]\.0.+/ or $MATLAB_INFO{"VERS"} =~ /6\.+/) {
			...
			$ome_matlab_h = $ome_matlab_h.$ome_matlab_h_augment;
		}
		
		open(FILE, ">", "src/matlab/ome-Matlab.h") or 
			print BOLD, "[FAILURE]", RESET, ".\n" and
			print $LOGFILE "Can't open src/matlab/ome-Matlab.h for writing: $!\n" and
			croak "Can't open src/matlab/ome-Matlab.h for writing: $!";
	
		print FILE $ome_matlab_h;
		close (FILE);
		print BOLD, "[SUCCESS]", RESET, ".\n";			

...
print "\nOME Test Case - OME Import\n";
print "--------------------------\n";

if (scalar(@ARGV) < 1) {
    print "Usage:  ImportOME [file list]\n\n";
    exit -1;
}
...
my $chainImporter = OME::Tasks::OMEImport->
  new(session => $session, debug => 0);
...
foreach (@ARGV) {
    ...
    my $newChains;
    eval {
        $newChains = $chainImporter->importFile($_,
                                                NoDuplicates => 1,

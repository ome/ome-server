...
sub generateOMEimage{
  ...
  my $isRGB;
	...
	foreach ($sizeX, $sizeY, $sizeZ, $numC, $numT, $bpp, $path) {
		unless (defined $_) {
			return undef;
		}
	}
...
sub initialize{
  ...
  $self->{inColor}=$isRGB;
	my @dim=();
  push(@dim, $sizeX , $sizeY, $sizeZ, $numC, $self->{T},$bpp);
	$self->{dim}=\@dim;
	if (defined $CBW){
  		$self->{CBW}=$CBW;
...
sub initCBW{
    ...
    my $B = ($CBW[$i*3+2] - $geomean) / $geosigma;
    $B=int($B*10000)/10000;
    push(@cCBW,$B);
  }
	return \@cCBW;
...
sub getConvertedCBW{
    ...
    $value=int($value);
    push(@cCBW,$value);
    if ($CBW[$i*3+2]==0){ $CBW[$i*3+2]=0.0001;}
    if ($geosigma==0){return undef};		# must be changed
    my $B=255/($geosigma*$CBW[$i*3+2]);
    $B=int($B*10000)/10000;
    push(@cCBW,$B);
  }
	return \@cCBW;

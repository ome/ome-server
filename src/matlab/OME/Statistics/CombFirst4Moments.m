...
%
% Example: hr = moments4Sig(im2double(imread(filename)));
%
function vec = moments4Sig(a),
[m,n] = size(a); if min(m,n)<=10, vec = zeros(1,16); mvec = zeros(1,16); return; end
I = (1:m)'*ones(1,n); J = ones(m,1)*(1:n); n2 = round(n/2); m2 = round(m/2); z4 = [0 0 0 0];
    % major diag ...
matr4moments = [];
for ii=(1-m):(round(m/10)):m, tmp = a(abs((I+ii)-J)<1);
 if isempty(tmp), matr4moments = [matr4moments; z4]; else, matr4moments = [matr4moments; get4scalMoments(tmp)]; end
end
vec = matr4moments_to_hist(matr4moments); matr4moments = [];
    % alternative diag ...
J1=fliplr(J);
for ii=(1-m):(round(m/10)):m, tmp = a(abs((I+ii)-J1)<1);
 if isempty(tmp), matr4moments = [matr4moments; z4]; else, matr4moments = [matr4moments; get4scalMoments(tmp)]; end
end
vec = [vec matr4moments_to_hist(matr4moments)]; matr4moments = [];
    % vertical comb ...
...
nbins = 3; vec = [];
for ii = 1:size(matr4moments,2),vec = [vec hist(matr4moments(:,ii),nbins)];end
return;

%:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
% compute mean, std, skewness, kurtosis of a vector
function m4 = get4scalMoments(vec),
ss = std(vec); if ~ss, m4 = [mean(vec) ss 0 0]; else, m4 = [mean(vec) ss skewness(vec) kurtosis(vec)];end
return;

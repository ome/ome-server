...
%% :: revision :: 07-01-2004 ::
%%
%% output: coeffMatrix (2D coeff matrix and reconstructed image)
%% input:  Im, N   (image and coeff order)
%%
%% Examples:
%% [coeffMatrix] = ChebyshevTransform(Im,170);
%% [coeffMatrix] = ChebyshevTransform(Im,20); 
%%
function [coeffMatrix] = ChebyshevTransform(Im,N);
	...
	y = 1:n_rows;
	y = 2 .* y ./ max(y) - 1;
	coeffMatrix = getChCoeff(getChCoeff(Im,x,N)',y,N);
	
	% if want to reconstruct image:
	if ~recYes
		im2 = [];
	else
		im2 = (recImg(x,N,recImg(y,N,coeffMatrix')'))';
	end
	
	% pack up coeff in 'Chebyshev space'
	if packYes
		[coeffMatrix_packed] = hist(coeffMatrix(:),30); 
		coeffMatrix = coeffMatrix_packed'; 
	end;
	
	% Cast the output back into a single if we expanded to double to reduce 
	% rounding errors
	if( singleInput )
		coeffMatrix = single( coeffMatrix );
	end;
return;
...
function c = getChCoeff(Im,x,N)
	for iy = 1:size(Im,1)
		c(iy,:) = getChCoeff1D(Im(iy,:),x,N);
	end;
return;

%% ChebyshevTransform for 1D: coefficients
function c = getChCoeff1D(f,x,N), 
	Tj = TNx(x,N);
	for jj = 1:N, 
		jx = jj-1; 
		tj = Tj(:,jj);
...
function im = recImg(x,N,cc)
	Tj = TNx(x,N); 
	for iy = 1:size(cc,1),
		for jx = 1:length(x), 
			im(iy,jx) = cc(iy,:) * Tj(jx,:)'; 
		end;
	end
return;

%% filter out some of coeff in 'Chebyshev space' (like in frequency space for FFT)
function [fc_packed,fc] = filterCoeff(c)
	N = size(c,1);
	mc = mean2(c);
	sc = std2(c);
	[cx,cy] = meshgrid(1:N);
	...
	fmax = max(cr(:));
	fc = c;
	origLen = length(find(fc));
	cutL = 0:.01:1;
	cutH = cutL+.005; 
	cutH = cutH(cutH<=1);
	cutL = cutL(1:length(cutH));
	cutL = cutL.*fmax;
	cutH = cutH.*fmax;
	cutL = cutL([1:3 4:3:end]);
	cutH = cutH([1:3 4:3:end]);
	str = [];
	for ind = 1:length(cutL)-1,
		L = cutL(ind);
		H = cutH(ind);
		str = [str '(' num2str(L) '<cr&cr<' num2str(H) ')|'];
	end;
	...
	nk = length(find(fc)); 
	fc = fc .* N ./ sqrt(nk);
	mkill2 = (abs(fc) < .5*sc) | (abs(fc) > 3*sc); 
	fc(mkill2) = 0;
	fc_packed = fc(fc>0);
return;

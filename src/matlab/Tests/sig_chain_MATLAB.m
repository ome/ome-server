...
% [445-478]    10^-12 <15, 15.8> FeatureStatistics(OtsuGlobalThreshold(im)))
...
function [signature_vector] = sig_chain_MATLAB (im); 
...
vec_6 = vd_Comb4Moments(concat_outputs (CombFirst4Moments(ChebyshevTransform(FrequencySpace2Pixels(FourierTransform(single(im)))))));
vec_7 = vd_Comb4Moments(concat_outputs (CombFirst4Moments(FrequencySpace2Pixels(FourierTransform(single(im))))));
...
[level, BinaryMask] = OtsuGlobalThreshold(im);
[Count, Euler, Centroid, AreaMin, AreaMax, AreaMean, AreaMedian, AreaVar, AreaHist, DistMin, DistMax, DistMean, DistMedian, DistVar, DistHist] = FeatureStatistics(logical(BinaryMask));
...
vec_22 = concat_outputs (MultiScaleHistograms(single(FrequencySpace2Pixels(FourierTransform(single(im))))));
vec_23 = concat_outputs (MultiScaleHistograms(single(WaveletSelector(WaveletSignatures(single(im))))));
vec_24 = concat_outputs (MultiScaleHistograms(single(WaveletSelector(WaveletSignatures(FrequencySpace2Pixels(FourierTransform(single(im))))))));
vec_25 = vd_RadonTextures(concat_outputs (RadonTransform(im)));
vec_26 = vd_RadonTextures(concat_outputs (RadonTransform(ChebyshevTransform(single(im)))));
vec_27 = vd_RadonTextures(concat_outputs (RadonTransform(ChebyshevTransform(FrequencySpace2Pixels(FourierTransform(single(im)))))));
vec_28 = vd_RadonTextures(concat_outputs (RadonTransform(FrequencySpace2Pixels(FourierTransform(single(im))))));
vec_29 = vd_TamuraTextures(concat_outputs (TamuraTextures(im)));
vec_30 = vd_TamuraTextures(concat_outputs (TamuraTextures(ChebyshevTransform(single(im)))));
vec_31 = vd_TamuraTextures(concat_outputs (TamuraTextures(ChebyshevTransform(FrequencySpace2Pixels(FourierTransform(single(im)))))));

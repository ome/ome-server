...
% [321-368]    EXACT  CombFirst4Moments(WaveletTransform((im)))
% [369-416]    EXACT  CombFirst4Moments(WaveletTransform(FourierTransform((im))))
% [417-444]    10^-12 <15, 15.8> EdgeStatistics(ImageGradient((im)))
% [445-478]    10^-12 <15, 15.8> ObjectStatistics(OtsuGlobalThresholdBackground(im)))
% [479-485]    EXACT  GaborTextures((im))
...
% [598-625]    10^-3 <5, 6.89> HaralickTexturesRI(WaveletTransform((im)))
% [626-653]    10^-3 <5, 6.89> HaralickTexturesRI(WaveletTransform(FourierTransform((im))))
...
% [750-773]    10^-7 <6, 8.95> MultiScaleHistograms(WaveletTransform((im)))
% [774-797]    10^-7 <6, 8.95> MultiScaleHistograms(WaveletTransform(FourierTransform((im))))
% [798-809]    EXACT RadonTransformStatistics((im))
% [810-821]    EXACT RadonTransformStatistics(ChebyshevTransform((im)))
% [822-833]    EXACT RadonTransformStatistics(ChebyshevTransform(FourierTransform((im))))
% [834-845]    EXACT RadonTransformStatistics(FourierTransform((im)))
...
% [870-875]    10^-4 <6, 9.69> TamuraTextures(WaveletTransform((im)))
% [876-881]    10^-1 <6, 9.69> TamuraTextures(WaveletTransform(FourierTransform((im))))
% [882-953]    10^-17 <16, 16> ZernikePolynomials((im))
% [954-1025]   10^-17 <16, 16> ZernikePolynomials(FourierTransform((im)))
...
WaveletSelector_WaveletTransform_single_im = WaveletSelector(WaveletTransform(single(im)));
WaveletSelector_WaveletTransform_FrequencySpace2Pixels_FourierTransform_single_im = WaveletSelector(WaveletTransform(FrequencySpace2Pixels_FourierTransform_single_im));
...
vec_8 = vd_Comb4Moments(concat_outputs (CombFirst4Moments(WaveletSelector_WaveletTransform_single_im)));
vec_9 = vd_Comb4Moments(concat_outputs (CombFirst4Moments(WaveletSelector_WaveletTransform_FrequencySpace2Pixels_FourierTransform_single_im)));
...
[level, BinaryMask] = OtsuGlobalThresholdBackground(im);
...
vec_11 = ro_ObjectStatistics(Count, Euler, Centroid, AreaMin, AreaMax, AreaMean, AreaMedian, AreaVar, AreaHist, DistMin, DistMax, DistMean, DistMedian, DistVar, DistHist);

vec_12 = concat_outputs (GaborTextures(single(im)));
...
vec_17 = vd_HaralickTexturesRI(concat_outputs (HaralickTexturesRI(WaveletSelector_WaveletTransform_single_im)));
vec_18 = vd_HaralickTexturesRI(concat_outputs (HaralickTexturesRI(WaveletSelector_WaveletTransform_FrequencySpace2Pixels_FourierTransform_single_im)));
...
vec_23 = concat_outputs (MultiScaleHistograms(single(WaveletSelector_WaveletTransform_single_im)));
vec_24 = concat_outputs (MultiScaleHistograms(single(WaveletSelector_WaveletTransform_FrequencySpace2Pixels_FourierTransform_single_im)));
vec_25 = vd_RadonTextures(concat_outputs (RadonTransformStatistics(im)));
vec_26 = vd_RadonTextures(concat_outputs (RadonTransformStatistics(ChebyshevTransform_single_im)));
vec_27 = vd_RadonTextures(concat_outputs (RadonTransformStatistics(ChebyshevTransform_FrequencySpace2Pixels_FourierTransform_single_im)));
vec_28 = vd_RadonTextures(concat_outputs (RadonTransformStatistics(FrequencySpace2Pixels_FourierTransform_single_im)));
...
vec_33 = vd_TamuraTextures(concat_outputs (TamuraTextures(WaveletSelector_WaveletTransform_single_im)));
vec_34 = vd_TamuraTextures(concat_outputs (TamuraTextures(WaveletSelector_WaveletTransform_FrequencySpace2Pixels_FourierTransform_single_im)));
vec_35 = concat_outputs (ZernikePolynomials(im));
vec_36 = concat_outputs (ZernikePolynomials(FrequencySpace2Pixels_FourierTransform_single_im));
...
function out = ro_ObjectStatistics(Count, Euler, Centroid, AreaMin, AreaMax, AreaMean, AreaMedian, AreaVar, AreaHist, DistMin, DistMax, DistMean, DistMedian, DistVar, DistHist)

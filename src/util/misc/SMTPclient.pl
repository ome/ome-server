...
# 
#
# Modified by Ilya Goldberg <igg@nih.gov> 10/2004 to accept stdin,
# and subject and headers flag
#
use strict;
use IO::Socket::INET;
use IO::Socket::SSL;
...
my ($user, $pass, $host, $port, 
    ...
    $hello_host, $from, @to, $datasrc,$subject,$headers,$stdin);
...
# --subject implies --headers
$headers = 1 if $subject;
...
sub run_smtp
{
	...
	if (defined ($datasrc) or $stdin)
	{
		if ($datasrc) {
			unless (open (MAIL, $datasrc))
			{
		...
		if ($code != 354)
		{
			...
			close (MAIL) if $datasrc;
			return 0;
		}

		if ($headers) {
			$sock->print ("Subject: $subject\n") if $subject;
			$sock->print ("From: $from\n") if $from;
			$sock->print ("To: ".join (" ",@to)."\n") if scalar @to;
		}
...
Author: Michal Ludvig <michal\@logix.cz> (c) 2003
...
Usage: smtp-client.pl [--options]
    ...
    --subject=<subject> Specify Subject header.
    --headers           Send headers (implied with --subject)

    --data=<filename>   Filename to paste after DATA command.

    --stdin             Transfer mail from STDIN

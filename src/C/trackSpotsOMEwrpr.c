...
void Set_Moved_To_Attributes (OMEdb dbHandle,long analysisID);
...
/*
* Insert the trajectory feature.
* We don't do an AutoInsert because the last feature in a trajectory has values of '--'
*/
	Set_Table_Attributes_Values ("DELTA",theColumnKey,attributesSQL,valuesSQL);

	bogusFeature = (strstr (valuesSQL,"--") != NULL);
	if (!bogusFeature)
		OME_Add_Attribute_Values (dbHandle,featureID,analysisID,"DELTA",attributesSQL,valuesSQL);
...
int i,j;
	...
	featureCount = OME_Get_Feature_Count (dbHandle,analysisID);

	featureID = OME_Get_Feature_IDs (dbHandle,analysisID);
	if (!featureID)
	{
		OME_Error (OME_FATAL_ERROR,"Could not read feature IDs (out of memory?)");
		OME_Exit (dbHandle);
	}

	timepoint = OME_Get_Feature_Attributes_As_Longs (dbHandle,featureID,analysisID,"TIMEPOINT","TIMEPOINT");
	if (!timepoint)
	{
		free (featureID);
		OME_Error (OME_FATAL_ERROR,"Could not read feature attributes (out of memory?)");
		OME_Exit (dbHandle);
	}

	trajectory_ID = OME_Get_Feature_Attributes_As_Longs (dbHandle,featureID,analysisID,"DELTA","TRAJECTORY_ID");
	if (!timepoint)
	{
		free (timepoint);
		free (featureID);
...
/*
* Update the trajectory IDs in a separate loop.  Seems safer that way.
* FIXME: The last timepoint's feature attributes aren't set right.
* These features have a trajectory attribute but no other delta attributes.
* Right now, these features do not have a DELTA attribute at all.
*/
	for (i=0;i < featureCount;i++)
	{
		OME_Set_Feature_Attribute_Long (dbHandle,featureID[i],analysisID,"DELTA","TRAJECTORY_ID",trajectory_ID[i]+1);
	}

	for (i=0;i < featureCount;i++)
		for (j=0;j<featureCount;j++)
			if ( (trajectory_ID[j] == trajectory_ID[i]) && (timepoint[j] == timepoint[i]+1) )
				OME_Set_Feature_Attribute_Long (dbHandle,featureID[i],analysisID,"DELTA","MOVED_TO",featureID[j]);
}
...
long Get_Analysis_ID (OMEdb dbHandle,int argc,char *argv[], 
		char *directoryPath, char *attrNames, char *attrValues,long *programID,long *datasetID)
{
char progName[]="trackSpots";  /* this is the OME program name. It is unique */
char message[256];
...
*/
	sprintf (attrNames,"TIME_START,TIME_STOP,WAVELENGTH,THRESHOLD,MIN_SPOT_VOL,INTENS_WEIGHT");
	sprintf (attrValues,"%d,%d,%d,'%s',%d,%f",tStart,tStop,spotWaveLngth,argv[3],minSpotVol,intnsWght);
...
char executableName[]="trackSpotsDB";              /* this is a path relative to the executable */
int i;
...
*/
	strcat (commandString," -db -tID -tt -th -tv -td -c 0 -i 0 -m 0 -g 0 -ms 0 -gs 0 -mc -v > ");
...
char columnKeyFile[]="trackspotsOMEcolumns";       /* this is a path relative to the executable */
...
fclose (stdout);
fclose (stderr);
stdout = fopen ("/tmp/trackSpotsOMEwrpr.stdout", "w");
stderr = fopen ("/tmp/trackSpotsOMEwrpr.stderr", "w");
*/
...
* By input here we mean the output of trackspots.  The output of this program is stored in the database.
*/
...
/*
* This selects attributes from the features generated in this analysis, and processes them
* to make links between a feature and its incarnation in subsequent timepoints.
* Sets the MOVED_TO attribute of DELTA, based on the same TRAJECTORY_ID in the next TIMEPOINT.
* Also, trajectory_IDs from findSpots/trackSpots begin at 0, not 1, so they get incremented.
*/
	Set_Moved_To_Attributes (dbHandle,analysisID);

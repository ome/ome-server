 ...
 * set_params
 *
 * Sets the parameters of an XML-RPC, XML document according to the length and
 ...
 */
static int
set_params (xmlNodePtr param, unsigned char *plist[], int len)
{
	xmlNodePtr value, string;
	int i;

	param = param->xmlChildrenNode;

	for (i = 0; i < len; i++) {
		if ((value = param->xmlChildrenNode) == NULL) {
			fprintf(stderr, \
					"Serious error finding parameter value nodes in XML file.\n");
			return -1;
		}
                   ...
                   unsigned char *password)
{
	...
	if (xmlStrcmp(cur->name, (const xmlChar *) "methodCall")) {
		fprintf(stderr,"document of the wrong type, root node != methodCall");
		xmlFreeDoc(doc);
		return NULL;
	}
	...
	/* Find params node and set parameter list */
	for ( ; cur != NULL ; cur = cur->next)
		if (!xmlStrcmp(cur->name, (const xmlChar *) "params"))
			set_params (cur, plist, 2);

	xmlDocDumpMemory (doc, &xml_out, &len);
...
user_data_parse_xml (const unsigned char *xml)
{
	...
	if (xmlStrcmp(cur->name, (const xmlChar *) "methodResponse")) {
		fprintf(stderr,"document of the wrong type, root node != methodResponse");
		xmlFreeDoc(doc);
		return NULL;
	}
 ...
 * puts them through their paces and returns a valid SID.
 *
 */
unsigned char *

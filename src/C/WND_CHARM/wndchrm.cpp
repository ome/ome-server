...
void ShowHelp()
{
   ...
   printf("usage: wndchrm [ train [-mtslh] <root directory> <output_file> ] | [ test [-trwpnfh] <input_file>] \n");
    ...
    {   if (strchr(argv[arg_index],'m')) multi_processor=1;
     ...
     {  if (train) 
        {  root_dir=argv[arg_index++];
            ...
            compute_features(root_dir, filename,MAX_CLASS_NUM, output_to_screen, tiles, multi_processor,large_set);
        }
        if (test)
       {   filename=argv[arg_index];
           split_and_test(filename, 100, method, tiles, split_ratio, max_features,splits_num,report);       
       }
   }  

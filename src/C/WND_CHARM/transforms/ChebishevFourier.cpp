...
*/
void ChebyshevFourier(ImageMatrix *Im, long N, double *coeff_packed, int packingOrder)
...
//     x[a]=-1+2/n);
   ...
   for (y=0;y<m;y++)
     for (x=0;x<n;x++)
       img[x*m+y]=Im->data[x][y].intensity;
   ...
   if (max!=min)
     ...
     {  if (coeff[a]==max) coeff_packed[31]+=1;
        ...
        coeff_packed[(int)(((coeff[a]-min)/(max-min))*32)]+=1;
     }

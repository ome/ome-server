...
{  double *xcoords,sum;
   ...
   for (col=0;col<image->width;col++)
     for (row=0;row<image->height;row++)
     {
        ...
        sum+=xcoords[row*image->width+col]*image->data[col][row].intensity;
     }
...
void mb_zernike(ImageMatrix *I, double D, double R, double *zvalues, long *output_size)
{  int rows,cols,n,l;
   double *Y,*X,*P,psum;
   ...
   for (y=0;y<rows;y++)
     for (x=0;x<cols;x++)
     if (I->data[x][y].intensity!=0)
     {  Y[size]=y+1;
        X[size]=x+1;
        P[size]=I->data[x][y].intensity;
        psum+=I->data[x][y].intensity;
        size++;
     }

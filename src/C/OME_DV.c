...
/*#########################*/
/*
* This function imports a DeltaVision file into the OME database.
* The datasetID should already exist in the database (i.e. as created by OME_Add_Dataset)
*/
long OME_Import_DV (OMEdb dbHandle, long datasetID)
{
FILE *DV_File;
DVhead head;
DVstack *theStack=NULL;
int time,i;
char attrValues[256];

	DV_File = OME_Open_Dataset_File (dbHandle,datasetID,"r");
	if (!DV_File)
	{
		OME_Error (OME_ERROR,"Could not open dataset file.");
		return (0);
	}

	if (!ReadDVHeader( &head, DV_File ))
	{
		fprintf (stderr,"Couldn't read DeltaVision file\n");
		exit (-1);
	}	
	
	sprintf (attrValues,"%ld,%ld,%ld,%ld,%d,%d,%f,%f,%f",datasetID,
		head.numCol,head.numRow,head.numImages / (head.NumWaves * head.numtimes),
		head.NumWaves,head.numtimes,head.xlen,head.ylen,head.zlen);
	OME_Modify_Dataset (dbHandle,datasetID,"DATASET_TYPE='DeltaVision',\
		ATTRIBUTES_TABLE='ATTRIBUTES_DATASET_XYZWT'");
	OME_Add_Attributes (dbHandle,"ATTRIBUTES_DATASET_XYZWT",
		"DATASET_ID,SIZE_X,SIZE_Y,SIZE_Z,NUM_WAVES,NUM_TIMES,\
		PIXEL_SIZE_X,PIXEL_SIZE_Y,PIXEL_SIZE_Z",attrValues);

	for (time=0;time<head.numtimes;time++)
	{
		if (theStack)
			{
			free (theStack->stack);
		...
		theStack = ReadDVstack (DV_File,&head,time);

	/*
	* Calculate statistics for the stack.
	*/
		for (i=0;i<theStack->nwaves;i++)
		{
			Calculate_Stack_Stats (theStack,i);

			sprintf (attrValues,"%ld,%d,%d,%d,%d,%f,%f,%f",datasetID,
				theStack->wave[i],time,
				theStack->min_i[i],theStack->max_i[i],theStack->mean_i[i],
				theStack->geomean_i[i],theStack->sigma_i[i]);

			OME_Add_Attributes (dbHandle,"STATS_XYZ","DATASET_ID,WAVELENGTH,TIMEPOINT,MIN,MAX,MEAN,GEOMEAN,SIGMA",
				attrValues);
		}
	}
